...
async function loadLang() {
  try {
    const { lang } = await chrome.storage.local.get(LANG_KEY);
...
async function getFavorites() {
  const { favorites = [] } = await chrome.storage.local.get(FAVORITES_KEY);
  return favorites
...
async function migrateAwayFromFolders() {
  const { favorites: raw = [] } = await chrome.storage.local.get(FAVORITES_KEY);
  ...
  if (JSON.stringify(final) !== before) {
    await chrome.storage.local.set({ [FAVORITES_KEY]: final });
  }
}
...
function renderChip(tab, label, urlCounts, favoritedUrls) {
...
function buildOverflowChips(hiddenTabs, urlCounts = {}, favoritedUrls = new Set()) {
  const hiddenChips = hiddenTabs.map(tab => {
    ...
    return renderChip(tab, label, urlCounts, favoritedUrls);
  }).join('');
