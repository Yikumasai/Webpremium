...
- 🔄 **Quản Lý Tab Thông Minh** - Tự động phát hiện và chuyển đến tab đã mở
...
#### 3. Quản Lý Tab Thông Minh
- **Phát Hiện Tab Trùng Lặp** - Tự động phát hiện tab có cùng URL
- **Tự Động Chuyển** - Khi nhấp vào liên kết đã mở, tự động chuyển đến tab hiện có
- **Di Chuyển Mượt Mà** - Tab đã tải trước di chuyển mượt mà đến cửa sổ chính
- **Tối Ưu Bộ Nhớ** - Giảm tab trùng lặp, giảm sử dụng bộ nhớ
...
- `Alt + P` - Nhanh chóng bật/tắt chức năng tải trước
- `Alt + C` - Xóa tất cả bộ nhớ cache tải trước
...
# Tải tiện ích mở rộng trong Chrome
# chrome://extensions/ → Chế độ nhà phát triển → Tải tiện ích đã giải nén
```
...
├── background.js          # Script dịch vụ nền
├── content.js            # Script nội dung
├── popup.html            # HTML cửa sổ popup
├── popup.js              # Script cửa sổ popup
