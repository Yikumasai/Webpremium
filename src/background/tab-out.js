...
import { CONTEXT_MENU } from '../shared/constants.js';
...
export async function addTabOutFavorite(url, title = '') {
  ...
  const stored = await chrome.storage.local.get(FAVORITES_KEY).catch(() => ({}));
  const favorites = Array.isArray(stored[FAVORITES_KEY]) ? stored[FAVORITES_KEY] : [];
  ...
  await chrome.storage.local.set({ [FAVORITES_KEY]: [...favorites, favorite] });
  return true;
}
...
function brandFromUrl(url) {
  try {
    ...
    const secondLevelTlds = ['co.uk', 'co.jp', 'com.cn', 'com.tw', 'com.au', 'com.hk', 'co.kr'];
    let brand = parts[0];
    if (parts.length >= 3 && secondLevelTlds.includes(parts.slice(-2).join('.'))) {
