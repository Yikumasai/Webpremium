...
function isLikelyOrphanBlankPreloadWindow(win) {
  ...
  return (
    width <= PRELOAD_WINDOW_OPTS.width + 120 &&
    height <= PRELOAD_WINDOW_OPTS.height + 140 &&
    Math.abs(left - PRELOAD_WINDOW_OPTS.left) <= 120 &&
    Math.abs(top - PRELOAD_WINDOW_OPTS.top) <= 120
  );
}
