...
export class Preloader {
  ...
  /**
   * 后台标签页方式预加载
   * @returns {Promise<{success: boolean, skippedExistingTab?: boolean}>} 是否成功
   */
  async preloadWithBackgroundTab(href, linkElement) {
    if (this.entries.has(href)) return { success: true };
    ...
    try {
      ...
      if (res.skipped && res.reason === 'existing-tab') {
        log.debug('目标已在当前窗口打开，跳过预加载', href);
        this.entries.delete(href);
        this.indicator.clear(linkElement);
        return { success: true, skippedExistingTab: true };
      }
