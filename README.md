...
- 🔄 **智能标签页去重** - 自动检测重复标签页，点击时跳转到已有标签页
- 🏠 **Tab-out 新标签页** - 美观的新标签页管理面板，支持收藏和标签页整理
- 📊 **实时统计** - 追踪预加载效果和节省的时间
- 🎨 **现代化界面** - 支持深色模式，界面简洁美观
- ⚙️ **高度可定制** - 丰富的设置选项，满足个性化需求
- 🌐 **多语言支持** - 支持简体中文、繁体中文、英文等多语言界面
...
- **LRU 缓存淘汰** - 超出上限时自动淘汰最久未使用的预加载

#### 2. 预加载模式
- **预加载窗口模式（推荐）** - 在独立的最小化窗口中预加载，完整加载页面，主窗口无干扰
- **iframe 预加载模式** - 轻量级预加载方式，兼容性好

#### 3. 智能标签页去重与跳转 (Smart Tab Dedup)
- **重复标签页检测** - 预加载前检测当前窗口是否已打开目标网页
- **自动跳转** - 点击已打开的链接时，自动跳转到现有标签页，避免重复打开
- **预加载跳过** - 如果目标页面已打开，跳过预加载直接跳转
- **无缝体验** - 自动聚焦到目标标签页和窗口

#### 4. Tab-out 新标签页管理
- **美观的新标签页** - 替换默认新标签页，显示功能丰富的管理面板
- **三栏布局** - 左侧收藏、中间打开的标签、右侧稍后阅读
- **域名分组** - 自动按域名分组显示打开的标签页
- **收藏功能** - 长期收藏常用网站，支持自定义图标
- **标签数量徽章** - 工具栏图标显示当前打开的标签页数量
- **重复标签检测** - 自动检测并提示关闭重复的新标签页
- **快捷操作** - 关闭标签、固定标签、添加到收藏等一键操作
- **深色模式** - 支持浅色/深色主题切换
- **多语言** - 支持中英文界面切换
...
- `Alt + P` - 快速开关预加载功能（需在设置中启用）
- `Alt + C` - 清理所有预加载缓存（需在设置中启用）
- 快捷键默认关闭，避免与系统或其它扩展冲突

### 右键菜单

- **预加载此链接** - 手动预加载选中的链接
- **在此网站启用/禁用预加载** - 快速切换当前网站的预加载状态
- **添加页面到收藏** - 将当前页面添加到 Tab-out 收藏
- **添加链接到收藏** - 将链接添加到 Tab-out 收藏
...
### 方法一：Chrome 网上应用店
搜索 link preloader：
> https://chromewebstore.google.com/detail/link-preloader/ajfjclgcglnjglkjgjcenkjjmipddnhj
...
- 打开设置面板
- 开启"启用快捷键"选项
- 使用 `Alt+P` 开关预加载，`Alt+C` 清理缓存
- 可在 Chrome 快捷键设置中自定义快捷键
...
| 预加载静音 | 预加载标签页默认静音 | 开启 | 开启 |
| 智能标签页去重 | 检测并跳转到已打开的标签页 | 开启 | 开启 |
| Tab-out 新标签页 | 启用新标签页管理面板 | 关闭 | 按需 |
| 快捷键 | 启用 Alt+P / Alt+C | 关闭 | 按需 |
...
### Q: 什么是智能标签页去重？
A: 当你要点击的链接已经在当前窗口打开时，插件会自动跳转到已有的标签页，而不是重新打开。这样可以避免重复标签页，节省内存。

### Q: 什么是 Tab-out？
A: Tab-out 是一个美观的新标签页管理面板，可以显示你打开的所有标签页（按域名分组）、收藏常用网站、管理稍后阅读等。

### Q: 预加载的标签页会发出声音吗？
A: 不会。预加载标签页默认静音，避免视频或直播网站自动播放出声。激活后你可以在地址栏点击解除静音。

### Q: 快捷键为什么不起作用？
A: 快捷键默认是关闭的，需要在设置中手动启用。这是为了避免与系统或其它扩展的快捷键冲突。
...
### v2.1.0 (当前版本)
- ✨ **智能标签页去重与跳转** - 预加载前检测当前窗口是否已打开目标网页，点击时优先跳转到已有标签页
- ✨ **Tab-out 新标签页管理** - 美观的新标签页面板，支持域名分组、收藏、标签数量徽章
- ✨ **预加载静音** - 预加载标签页默认静音，避免自动播放出声
- ✨ **快捷键冲突检测** - 自动检测并提示快捷键冲突
- ✨ **多语言支持** - 支持简体中文、繁体中文、英文界面
- ✨ **LRU 缓存淘汰** - 超出预加载上限时自动淘汰最久未使用的内容
- ✨ **默认网站规则** - 预置抖音等视频网站默认禁用预加载
- 🔧 优化预加载窗口管理，支持多窗口复用
- 🔧 优化标签页追踪和清理逻辑
...
├── background.js          # 后台服务脚本入口
├── content.js             # 内容脚本入口
├── popup.html             # 弹出窗口 HTML
├── popup.js               # 弹出窗口脚本入口
├── popup.css              # 弹出窗口样式
├── index.html             # Tab-out 新标签页 HTML
├── app.js                 # Tab-out 新标签页逻辑
├── style.css              # Tab-out 新标签页样式
├── icons/                 # 图标文件
│   ├── icon16.png
│   ├── icon32.png
│   ├── icon48.png
...
├── src/                   # 模块化源码
│   ├── background/        # 后台模块
│   │   ├── preload-window.js   # 预加载窗口管理
│   │   ├── router.js           # 消息路由
│   │   ├── settings-store.js   # 设置存储
│   │   ├── site-rules.js       # 网站规则
│   │   ├── stats.js            # 统计
│   │   ├── tab-deduper.js      # 标签页去重
│   │   ├── tab-out.js          # Tab-out 功能
│   │   └── tab-tracker.js      # 标签页追踪
│   ├── content/           # 内容脚本模块
│   │   ├── indicator.js        # 视觉指示器
│   │   ├── link-tracker.js     # 链接追踪
│   │   ├── main.js             # 主入口
│   │   ├── network-aware.js    # 网络感知
│   │   ├── preloader.js        # 预加载器
│   │   └── settings.js         # 设置管理
│   ├── popup/             # 弹窗模块
│   │   ├── api.js              # API 封装
